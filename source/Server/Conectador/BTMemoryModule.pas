    ...
    begin
      ...
      while l_relocation.VirtualAddress > 0 do
        begin
          ...
          for l_i := 0 to (trunc(((l_relocation.SizeOfBlock - IMAGE_SIZEOF_BASE_RELOCATION) / 2)) - 1) do
            begin
...
begin
  ...
  if (l_directory.Size > 0) then
    begin
      ...
      while (not IsBadReadPtr(l_importDesc, SizeOf(TImageImportDescriptor))) and (l_importDesc.Name <> 0) do
        begin
    ...
    begin
      ...
      if l_size = 0 then
        begin
          ...
          if l_size > 0 then
            begin
              if not VirtualProtect(Pointer(l_section.Misc.PhysicalAddress), l_section.SizeOfRawData, l_protect, @l_oldProtect) then
                begin
