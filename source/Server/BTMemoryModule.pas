...
function BTMemoryLoadLibary(var f_data: Pointer;
  const f_size: Int64): PBTMemoryModule; stdcall;
// return value is nil if function fails
function BTMemoryGetProcAddress(var f_module: PBTMemoryModule;
  const f_name: PChar): Pointer; stdcall;
// free module
...
function GetHeaderDictionary(f_module: PBTMemoryModule;
  f_idx: Integer): PImageDataDirectory; stdcall;
begin
...
procedure CopySections(const f_data: Pointer; const f_old_headers: TImageNtHeaders;
  f_module: PBTMemoryModule); stdcall;
var
  l_size, i: Integer;
  l_codebase: Pointer;
  l_dest: Pointer;
...
begin
  ...
  for i := 0 to f_module.headers.FileHeader.NumberOfSections - 1 do
    begin
      // section doesn't contain data in the dll itself, but may define
      // uninitialized data
      if (l_section.SizeOfRawData = 0) then
        begin
          l_size := f_old_headers.OptionalHeader.SectionAlignment;
          if l_size > 0 then
            begin
              l_dest := VirtualAlloc(Pointer(Cardinal(l_codebase) + l_section.VirtualAddress),
                l_size, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
      ...
      l_dest := VirtualAlloc(Pointer(Cardinal(l_codebase) + l_section.VirtualAddress),
        l_section.SizeOfRawData, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
      CopyMemory(l_dest, Pointer(Longword(f_data) + l_section.PointerToRawData),
        l_section.SizeOfRawData);
      l_section.Misc.PhysicalAddress := Cardinal(l_dest);
    ...
    begin
      ...
      while l_relocation.VirtualAddress > 0 do
        begin
          ...
          for l_i := 0 to (trunc(((l_relocation.SizeOfBlock - IMAGE_SIZEOF_BASE_RELOCATION) /
            2)) - 1) do
            begin
...
begin
  ...
  if (l_directory.Size > 0) then
    begin
      l_importDesc := PImageImportDescriptor(Cardinal(l_codeBase) +
        l_directory.VirtualAddress);
      while (not IsBadReadPtr(l_importDesc, SizeOf(TImageImportDescriptor))) and
        (l_importDesc.Name <> 0) do
        begin
          l_handle := LoadLibrary(PChar(Cardinal(l_codeBase) + l_importDesc.Name));
          if (l_handle = INVALID_HANDLE_VALUE) then
            begin
              lastErrStr := 'BuildImportTable: can''t load library: ' +
                PChar(Cardinal(l_codeBase) + l_importDesc.Name);
              Result := False;
              Exit;
            end;
          ...
          f_module.modules := ReallocMemory(f_module.modules,
            ((f_module.numModules + 1) * (SizeOf(HMODULE))));
          if f_module.modules = nil then
            begin
          ...
          if l_importDesc.OriginalFirstThunk <> 0 then
            begin
          ...
          while l_thunkRef^ <> 0 do
            begin
              if GetImageSnapByOrdinal(l_thunkRef^) then
                l_funcRef^ := Cardinal(GetProcAddress(l_handle,
                  PChar(GetImageOrdinal(l_thunkRef^))))
              else
                begin
                  CopyMemory(@l_thunkData, Pointer(Cardinal(l_codeBase) + l_thunkRef^),
...
begin
  ...
  if (SC and IMAGE_SCN_MEM_EXECUTE) <> 0 {//E ?} then
    if (SC and IMAGE_SCN_MEM_READ) <> 0 {//ER ?} then
      if (SC and IMAGE_SCN_MEM_WRITE) <> 0 {//ERW ?} then
        Result := Result or PAGE_EXECUTE_READWRITE
      else
        Result := Result or PAGE_EXECUTE_READ
    else if (SC and IMAGE_SCN_MEM_WRITE) <> 0 {//EW?} then
      Result := Result or PAGE_EXECUTE_WRITECOPY
    else
      Result := Result or PAGE_EXECUTE
  else if (SC and IMAGE_SCN_MEM_READ) <> 0 {// R?} then
    if (SC and IMAGE_SCN_MEM_WRITE) <> 0 {//RW?} then
      Result := Result or PAGE_READWRITE
    else
      Result := Result or PAGE_READONLY
  else if (SC and IMAGE_SCN_MEM_WRITE) <> 0 {//W?} then
...
begin
  l_section := GetImageFirstSection(f_module.headers);
  for l_i := 0 to f_module.headers.FileHeader.NumberOfSections - 1 do
    begin

      if (l_section.Characteristics and IMAGE_SCN_MEM_DISCARDABLE) <> 0 then
        begin
          // section is not needed any more and can safely be freed
          VirtualFree(Pointer(l_section.Misc.PhysicalAddress), l_section.SizeOfRawData,
            MEM_DECOMMIT);
          Inc(Longword(l_section), SizeOf(TImageSectionHeader));
          Continue;
        end;
      ...
      if l_size = 0 then
        begin
          if (l_section.Characteristics and IMAGE_SCN_CNT_INITIALIZED_DATA) <> 0 then
            begin
          ...
          if l_size > 0 then
            begin
              if not VirtualProtect(Pointer(l_section.Misc.PhysicalAddress),
                l_section.SizeOfRawData, l_protect, @l_oldProtect) then
                begin
...
begin
  ...
  try
    ...
    CopyMemory(@l_old_header, Pointer(Longint(f_data) + l_dos_header._lfanew),
    ...
    l_code := VirtualAlloc(Pointer(l_old_header.OptionalHeader.ImageBase),
      l_old_header.OptionalHeader.SizeOfImage, MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if l_code = nil then
    ...
    VirtualAlloc(l_code, l_old_header.OptionalHeader.SizeOfImage,
      MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    // commit memory for headers
    l_headers := VirtualAlloc(l_code, l_old_header.OptionalHeader.SizeOfHeaders,
      MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    // copy PE header to code
    CopyMemory(l_headers, f_data, (Cardinal(l_dos_header._lfanew) +
    ...
    l_locationdelta := Cardinal(Cardinal(l_code) -
      l_old_header.OptionalHeader.ImageBase);
    if l_locationdelta <> 0 then
      PerformBaseRelocation(l_result, l_locationdelta);
    // load required dlls and adjust function table of imports
    if not BuildImportTable(l_result) then
      begin
    ...
    if (l_result.headers.OptionalHeader.AddressOfEntryPoint) <> 0 then
      begin
        @l_DllEntry := Pointer(Cardinal(l_code) +
          l_result.headers.OptionalHeader.AddressOfEntryPoint);
        if @l_DllEntry = nil then
          begin
...
begin
  ...
  for l_i := 0 to l_exports.NumberOfNames - 1 do
    begin
      if StrComp(f_name, PChar(Cardinal(l_codeBase) + l_nameRef^)) = 0 then
        begin
          l_idx := l_ordinal^;
          break;
        end;
  ...
  if (Cardinal(l_idx) > l_exports.NumberOfFunctions - 1) then
    begin
      lastErrStr := 'BTMemoryGetProcAddress: name <-> ordinal number don''t match';
      Exit;
    end;
  // AddressOfFunctions contains the RVAs to the "real" functions
  l_temp := Pointer(Cardinal(l_codeBase) + Cardinal(l_exports.AddressOfFunctions) +
    Cardinal((l_idx * 4)));
...
begin
  ...
  if l_module <> nil then
    begin
      if l_module.initialized then
        begin
          @l_DllEntry := Pointer(Cardinal(l_module.codeBase) +
            l_module.headers.OptionalHeader.AddressOfEntryPoint);
          l_DllEntry(Cardinal(l_module.codeBase), DLL_PROCESS_DETACH, nil);
          l_module.initialized := False;
          // free previously opened libraries
          for l_i := 0 to l_module.numModules - 1 do
            begin
